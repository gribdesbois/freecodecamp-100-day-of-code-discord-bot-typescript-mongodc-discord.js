...
export const edit: Command = {
  ...
  run: async (interaction) => {
    ...
    if (!targetMessage) {
      await interaction.editReply({
        content:
          'That does not appear to be a valid mesage ID. Be sure that you are using this command in the same channel as the message',
      })
      return
    }
    ...
    if (targetEmbed.author?.name !== user.tag) {
      await interaction.editReply({
        content:
          'This does not appear to be your 100 Days of Code post. You cannt edit it',
      })
    }
